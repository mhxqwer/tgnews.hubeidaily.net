 ...
 * 最后修正：2009-12-18
 **/
var IMAGESPATH = '/r/cms/www/default/libs/zDialog/skins/blue/'; //
/*************************一些公用方法和属性****************************/
...
Dialog.attachBehaviors = function () {
    if (isIE) {
        $.addEvent('keydown', Dialog.onKeyDown);
        $.addEvent('resize', Dialog.resetPosition);//D:\workspace\HeiBeiMixedMedia\code\MixedMedia\HuBei\Content\js\base\commonality.js
...
Dialog.setBgDivSize = function () {
    ...
    if (Dialog.bgDiv) {
        if (isIE6) {
            Dialog.bgDiv.style.height = (bd.clientHeight - 1) + "px";
            Dialog.bgDiv.style.top = bd.scrollTop + "px";
            Dialog.bgDiv.childNodes[0].style.display = "none";//让div重渲染,修正IE6下尺寸bug
            Dialog.bgDiv.childNodes[0].style.display = "";
        } else {
            Dialog.bgDiv.style.height = (bd.scrollHeight - 1) + "px";
        }
    }
};
...
Dialog.prototype.create = function () {
    ...
    if (DialogDivHeight > bd.clientHeight) this.Height = Math.round(bd.clientHeight - 47 - (this.ShowButtonRow ? 40 : 0) - (this.ShowMessageRow ? 50 : 0));
    var displayValue = this.ShowCloseButton ? 'inline-block' : 'none';
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="13" height="28" style="display:none; background-image: url(' + IMAGESPATH + 'dialog_lt.png) !important;background: url(' + IMAGESPATH + 'dialog_lt.gif) no-repeat 0 0;"><div style="width: 13px;"></div></td>\
      <td height="28" style="border-radius:3px 3px 0 0; background-color:#5F8CC1;"><div style="padding: 4px 0 0 4px; float: left; font-weight: bold; color:#fff;"><img align="absmiddle" style="margin-right:5px" src="' + IMAGESPATH + 'icon_dialog.gif"/><span style="margin-top:2px" id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn_over.gif)\'" style="margin-top:6px; position: relative; cursor: pointer; float: right; height: 17px; width: 28px; background-image: url(' + IMAGESPATH + 'dialog_closebtn.gif);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
      <td width="13" height="28" style="display:none; background-image: url(' + IMAGESPATH + 'dialog_rt.png) !important;background: url(' + IMAGESPATH + 'dialog_rt.gif) no-repeat right 0;"><div style="width: 13px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td width="13" style="display:none; background-image: url(' + IMAGESPATH + 'dialog_mlm.png) !important;background: url(' + IMAGESPATH + 'dialog_mlm.gif) repeat-y left;"></td>\
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          ...
          <tr>\
            <td valign="top" align="center"><div id="_Container_' + this.ID + '" style="position: relative; width: 100%; height: ' + this.Height + 'px;background-color:#fff;' + this.OtherStyle + '">\
	    ...
	    return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: center; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle btn btn-small btn-default" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" style="display:' + displayValue + ';" class="buttonStyle btn btn-small btn-default" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
      <td width="13" style="display:none; background-image: url(' + IMAGESPATH + 'dialog_mrm.png) !important;background: url(' + IMAGESPATH + 'dialog_mrm.gif) repeat-y right;"></td>\
    </tr>\
    <tr>\
      <td width="13" height="13" style="display:none;background-image: url(' + IMAGESPATH + 'dialog_lb.png) !important;background: url(' + IMAGESPATH + 'dialog_lb.gif) no-repeat 0 bottom;"></td>\
      <td style="display:none;background-image: url(' + IMAGESPATH + 'dialog_cb.png) !important;background: url(' + IMAGESPATH + 'dialog_cb.gif) repeat-x bottom;"></td>\
      <td width="13" height="13" style="display:none;background-image: url(' + IMAGESPATH + 'dialog_rb.png) !important;background: url(' + IMAGESPATH + 'dialog_rb.gif) no-repeat right bottom;"><a onfocus=\'$id("_forTab_' + this.ID + '").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
'
    //setTimeout(function(){
    //    var html_h=$("#_DialogFrame_0").contents().find("html").height();
    //    //console.log(html_h);
    //    $("#_Container_0").height(html_h);
    //},300);
...
Dialog.prototype.close = function () {
    ...
    if (topWin.Dialog._Array.length > 0) {
        ...
        Dialog.bgDiv.style.zIndex = "9900";
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
        ...
        bgdiv.style.cssText = "position:absolute;left:0px;top:0px;width:100%;height:100%;z-index:9900";
...
Dialog.alert = function (msg, func, isAutoClose, w, h) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
    ...
    if (this.ID == undefined || this.ID == "undefined") {
		...
		<tr><td align="right"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:14px;padding: 0px 15px;font-family: "宋体";color: #535353;">' + msg + '</td></tr>\
	</table>';
    } else {
        diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
...
if (isIE) {
    ...
    $.addEvent('load', Dialog.attachBehaviors);
} else {
